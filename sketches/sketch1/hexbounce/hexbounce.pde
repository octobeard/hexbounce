...
void setup() {
  ...
  fullScreen();
	...
	pool2.autoAddToStage().add(new HShape(pathDATA + "hexagon.svg"))
	...
	pool1.autoAddToStage().add(new HShape(pathDATA + "hexagon.svg"))
		 .layout(new HGridLayout()
			.startX(10)
			.startY(40)
			.spacing(myStageW/20,myStageW/20)
			.cols(20)
		 )
		 .onCreate(new HCallback() {
				public void run(Object obj) {
					HShape d = (HShape) obj;
					int initSizeOffset = (int) random(myAudioRange*6) + 50;
					d
						.enableStyle(false)
						.stroke(#000000)
						.anchorAt(H.CENTER)
